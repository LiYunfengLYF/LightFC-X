...
class ExperimentRGBPS(object):
    ...
    def run(self, tracker, divided_init=True):
        ...
        for seq_num, sequence in enumerate(self.dataset):
            ...
            for item_num, data in tqdm(enumerate(sequence), total=len(sequence),
                ...
                # read img
                light_img = etrack.imread(data['light'])
                sonar_img = etrack.imread(data['sonar'])
        ...
        etrack.greenprint(f'Finish running {tracker.name} on {self.dataset.name}')

    def eval(self, tracker_name, protocol=1, epoch_num=None):
        time.sleep(0.01)
        ...
        for seq_num, sequence in enumerate(self.dataset):
            if epoch_num is not None:
                seq_save_root = os.path.join(self.save_root, (tracker_name + f'_ep{epoch_num}'), sequence.name)
            else:
                seq_save_root = os.path.join(self.save_root, (tracker_name), sequence.name)

            rgb_results = etrack.txtread(os.path.join(seq_save_root, 'light.txt'))
            s_results = etrack.txtread(os.path.join(seq_save_root, 'sonar.txt'))
        ...
        if epoch_num is not None:
            etrack.greenprint(f'Eval [{tracker_name}_ep{epoch_num}] performance of [{self.dataset.name}]: ')
        else:
            etrack.greenprint(f'Eval [{tracker_name}] performance of [{self.dataset.name}]: ')
    ...
    def run_sequence(self, sequence, tracker_params, tracker_factory, epoch_num):
        tracker = tracker_factory(tracker_params)
        rgb_boxes, s_boxes = [], []

        seq_save_root = os.path.join(self.save_root, (tracker.name + f'_ep{epoch_num}'), sequence.name)
        seq_rgb_save = os.path.join(seq_save_root, 'light.txt')
        seq_s_save = os.path.join(seq_save_root, 'sonar.txt')

        if os.path.exists(seq_rgb_save) and os.path.exists(seq_s_save):
            finish_num = int(len(os.listdir(os.path.join(self.save_root, (tracker.name + f'_ep{epoch_num}')))))
            etrack.greenprint(f'\t[{finish_num}/50]Finish running {tracker.name} on {sequence.name}')
            return 0
        else:
            try:
                os.makedirs(seq_save_root)
        ...
        finish_num = int(len(os.listdir(os.path.join(self.save_root, (tracker.name + f'_ep{epoch_num}')))))
        etrack.greenprint(f'[{finish_num}/50]Running Tracker: {tracker.name} Sequence: {sequence.name}')
    ...
    def multi_run(self, tracker_params, tracker_factory, threads, epoch_num):
        param_list = [(seq, tracker_params, tracker_factory, epoch_num) for seq in (self.dataset)]
