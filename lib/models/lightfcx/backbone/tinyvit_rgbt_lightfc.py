...
from timm.models.layers import DropPath as TimmDropPath, to_2tuple
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        img_v = self.patch_embed(x[0])
        img_i = self.patch_embed(x[1])
...
class TinyViT_add2cat(nn.Module):
    def __init__(self, img_size=256, in_chans=3, num_classes=1000,
                 embed_dims=[96, 192, 384, 768], depths=[2, 2, 6, 2],
                 num_heads=[3, 6, 12, 24],
                 window_sizes=[7, 7, 14, 7],
    ...
    def forward_features(self, x):
        ...
        img_fusion = torch.concat((img_v, img_i), dim=2)
...
class TinyViT_add2cat_v2(nn.Module):
    def __init__(self, img_size=256, in_chans=3, num_classes=1000,
                 embed_dims=[96, 192, 384, 768], depths=[2, 2, 6, 2],
                 num_heads=[3, 6, 12, 24],
                 window_sizes=[7, 7, 14, 7],
...
def _create_tiny_vit_add2cat(variant, pretrained=False, **kwargs):
    # pretrained_type: 22kto1k_distill, 1k, 22k_distill
    pretrained_type = kwargs.pop('pretrained_type', '22kto1k_distill')
    assert pretrained_type in ['22kto1k_distill', '1k', '22k_distill'], \
        'pretrained_type should be one of 22kto1k_distill, 1k, 22k_distill'
    ...
    if timm.__version__ >= "0.6":
        return build_model_with_cfg(
            TinyViT_add2cat, variant, pretrained,
            pretrained_cfg=cfg,
            pretrained_filter_fn=_pretrained_filter_fn,
            **kwargs)
    else:
...
def _create_tiny_vit_add2catv2(variant, pretrained=False, **kwargs):
    # pretrained_type: 22kto1k_distill, 1k, 22k_distill
    pretrained_type = kwargs.pop('pretrained_type', '22kto1k_distill')
    assert pretrained_type in ['22kto1k_distill', '1k', '22k_distill'], \
        'pretrained_type should be one of 22kto1k_distill, 1k, 22k_distill'
...
@MODEL_REGISTRY.register()
def tiny_vit_5m_224_add2cat(pretrained=False, **kwargs):
    model_kwargs = dict(
        # embed_dims=[64, 128, 160, 320],
        # depths=[2, 2, 6, 2],
        # num_heads=[2, 4, 5, 10],
    ...
    model_kwargs.update(kwargs)
    return _create_tiny_vit_add2cat('tiny_vit_5m_224', False, **model_kwargs)



@MODEL_REGISTRY.register()
def tiny_vit_11m_224_add2cat(pretrained=False, **kwargs):
    model_kwargs = dict(
    ...
    model_kwargs.update(kwargs)
    return _create_tiny_vit_add2cat('tiny_vit_11m_224', False, **model_kwargs)
