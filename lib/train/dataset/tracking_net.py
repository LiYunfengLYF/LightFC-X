...
class TrackingNet(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, set_ids=None, data_fraction=None, rgbs_mode=False, ):
        """
        args:
        ...
        self.rgbs_mode = rgbs_mode
        if rgbs_mode:
            self.rgbs_trans = cv2.saliency.StaticSaliencySpectralResidual_create()
    ...
    def _get_frame_rgbs(self, seq_id, frame_id):
        set_id = self.sequence_list[seq_id][0]
        vid_name = self.sequence_list[seq_id][1]
        frame_path = os.path.join(self.root, "TRAIN_" + str(set_id), "frames", vid_name, str(frame_id) + ".jpg")
        return cv2.imread(frame_path, 0)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        if self.rgbs_mode:
            frame_list = []
            for num, f_id in enumerate(frame_ids):
                if num == 0:
                    frame_list.append(self._get_frame(seq_id, f_id))
                else:
                    rgb_img = self._get_frame_rgbs(seq_id, f_id)
                    _, new_img = self.rgbs_trans.computeSaliency(rgb_img)
                    frame_list.append(np.stack((new_img, new_img, new_img), axis=-1))
        else:
