...
def parameters(yaml_name: str, env_num=0):
    ...
    yaml_file = os.path.join(prj_dir, 'experiments/rgbter_light/%s.yaml' % yaml_name)
    ...
    params.checkpoint = os.path.join(save_dir, "checkpoints/train/rgbter_light/%s/%s_ep%04d.pth.tar" %
                                     (yaml_name, params.cfg.MODEL.NETWORK, params.cfg.TEST.EPOCH))
    params.base_checkpoint = os.path.join(save_dir,
                                          "checkpoints/train/rgbter_light/baselinev2_ep45/RGBTer_lightfc_ep0045.pth.tar")
