...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='rgbd'):

    torch.set_num_threads(1)
    save_root = os.path.join('', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = rgbter_light(tracker_name=tracker_name, para_name=para_name)
