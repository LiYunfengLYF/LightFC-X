...
from lib.test.tracker.rgbter_light import RGBTer_light
from lib.test.parameter.rgbter_light import parameters
...
'''
python tracking/test_rgbe.py --script_name rgbt_light --yaml_name ablation_update_RGBE
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    if script_name == 'rgbt_light':
        params = parameters(yaml_name, env_num=5)
        params.debug = 0
        tracker = RGBE(tracker=RGBTer_light(params, dataset_name))
...
if __name__ == '__main__':
    ...
    if dataset_name == 'VisEvent':
        seq_home = '/home/datasets/visevent/test_subset'
